...
from src.identifier import identify_fronts, identify_fronts_f_diagnostic
...
def main():
    ...
    plot_time_range = ("2025-10-05", "2025-10-10") 
    ...
    if not os.path.exists(mask_output_path):
        ...
        try:
            ...
            # F-diagnostic
            ds_t = xr.open_dataset(os.path.join(output_location, "era5_t.nc"))
            ds_u = xr.open_dataset(os.path.join(output_location, "era5_u.nc"))
            ds_v = xr.open_dataset(os.path.join(output_location, "era5_v.nc"))

            logger.info("Calculating front masks with F-diagnostic...")
            # The threshold is set to 1.0 by default, but it can be adjusted according to region (e.g., 1.5 or 2.0).
            front_mask_da = identify_fronts_f_diagnostic(ds_t, ds_u, ds_v, threshold=1.0)
    ...
    try:
        ...
        plot_front_samples(
            ...
            max_plots=100 # Change this to generate more/fewer frames
        )
    except Exception as e:
